...
router.get('/walkers/summary', async function (req, res, next) {
    try {
        const [completedWalks] = await db.query(
            `SELECT U.username AS walker_username, COUNT(WalkRatings.rating_id) AS total_ratings,
            ...
            WHERE WalkRequests.status = 'completed' AND WA.walker_id = U.user_id) AS completed_walks
            From Users U
